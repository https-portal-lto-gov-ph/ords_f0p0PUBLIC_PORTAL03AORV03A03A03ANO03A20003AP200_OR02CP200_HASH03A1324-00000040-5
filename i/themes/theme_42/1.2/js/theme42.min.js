apex.theme42 = {},
    function(ut, $) {
        "use strict";
        var treeNav$, pageTitle$, pageBody$, bodyContent$, bodyContentOffset$, mainBody$, sideCol$, header$, footer$, actionsCol$, TREE_NAV = ut.TREE_NAV = "#t_TreeNav",
            ...
            T_ALERT_SUCCESS = (ut.T_MODAL_BODY = ".t-Dialog-body", "#t_Alert_Success"),
            APEX_SUCCESS_MESSAGE = "#APEX_SUCCESS_MESSAGE",
            ITEM_DROPZONE = "span.apex-item-file-dropzone",
            marqueeRDS$ = (ut.dialogHeight = 0, $(".t-Body-info .apex-rds-container")),
            baseRDS$ = $(".apex-rds-container"),
            init = ut.init = {};
        ut.util = {}, ut.util.scrollTo = function(id) {
            var anchor, elem$, elemOffset;
            id && (anchor = id.indexOf("#") > -1 ? id : "#" + id, elem$ = $(anchor), elem$[0] && (elemOffset = elem$.offset().top - apex.theme.defaultStickyTop(), $("html, body", apex.gPageContext$).animate({
                scrollTop: elemOffset
            })))
        }, ut.util.mq = function(mediaQueryString) {
            var matchMedia = window.matchMedia || window.msMatchMedia;
            return matchMedia ? matchMedia(mediaQueryString).matches : !1
        };
        ...
        var getFixedHeight = ut.getFixedHeight = function() {
            var headerHeight = $("header").outerHeight(),
                rdsHeight = 0;
            return marqueeRDS$.length > 0 && (rdsHeight += marqueeRDS$.outerHeight()), screenIsSmall() ? pageBody$.hasClass("js-HeaderContracted") ? rdsHeight : headerHeight + rdsHeight : ($(".js-stickyTableHeader").length > 0 && (rdsHeight -= 1), ut.getTitleHeight() + headerHeight + rdsHeight)
        };
        apex.theme.defaultStickyTop = getFixedHeight, ut.sticky = function(selector) {
            ...
            children$.each(function() {
                ...
                if (-1 === label.indexOf("<span class='" + labelClass + "'>")) {
                    ...
                    null !== match && match.length > 1 && ("" === match[1] ? this.innerHTML = label.replace(regex, "") : (label = label.replace(/\[.*\]/, "") + "<span class='" + labelClass + "'>" + match[1] + "</span>", this.innerHTML = label))
                }
            })
        };
        var resetHeaderOffset = ut.resetHeaderOffset = function() {
            var contentOffset, sideColTop, pageTitleHeight = ut.getTitleHeight(),
                headerHeight = header$.outerHeight();
            ut.util.mq("(min-width: 641px)") ? (contentOffset = pageTitleHeight, sideColTop = headerHeight + pageTitleHeight) : (contentOffset = 0, sideColTop = 0), mainBody$.css({
                ...
                if (pOptions && pOptions.autoDismiss && suc$[0]) {
                    var hide = function() {
                            timeOut = setTimeout(function() {
                                apex.message.hidePageSuccess(), suc$.off(), $(document).off("click", hide)
                            }, pOptions.duration || 3e3)
                        },
                        clear = function() {
                            clearTimeout(timeOut)
                        };
                    hide(), suc$.on("click", function(e) {
                        e.stopPropagation(), ut.configureSuccessMessages.clicked = !0, clear()
                    }).on("mouseover", clear).on("mouseout", function() {
                        ut.configureSuccessMessages.clicked || hide()
                    }), $(document).on("click", hide), suc$.find(".t-Button--closeAlert").on("focus", clear), ut.configureSuccessMessages.options = pOptions
                }
            },
            function() {
                var pages = ut.pages = {
                ...
                ut.initializePage = function() {
                    var wrapFunc = function(key) {
                            return function() {
                                ...
                                void 0 !== onReady && onReady(), void 0 !== onTheme42Ready && $(window).on("theme42ready", function() {
                                    onTheme42Ready()
                                })
                            }
                        },
                        returnPages = {};
                    for (var key in pages) pages.hasOwnProperty(key) && (returnPages[key] = wrapFunc(key));
                    return returnPages
                }()
            }(),
            function() {
                ...
                ut.delayResize = function() {
                    clearTimeout(delayResize), delayResize = function() {
                        $("body").trigger("theme42delayresizefinish"), $(".js-stickyWidget-toggle").each(function() {
                ...
                if (!(treeNav$.length > 0 && $.menu)) {
                    ...
                    menubar$.is(":data('ui-menu')") ? render() : $(window).on("menucreate apexwindowresized", menubar$, function() {
                        render()
                    })
                }
                ...
                apex.message.setThemeHooks({
                    ...
                    beforeShow: function(pMsgType) {
                        if (pMsgType === apex.message.TYPE.SUCCESS) {
                            ...
                            msg$.removeClass("animate-hide"), opt && ut.configureSuccessMessages(opt)
                        }
                    }
                })
            }, init.items = function() {
                $("input[type=file].apex-item-file").each(function() {
                    var that$ = $(this);
                    that$.next(ITEM_DROPZONE).html('<span class="apex-item-file-dropzone-label">' + that$.data("caption") + '</span><span class="apex-item-file-dropzone-icon"><span class="fa fa-folder-search" aria-hidden="true"></span></span>')
                }).on("change", function(e) {
                    var files = this.files,
                        fileName = "",
                        that$ = $(this),
                        dropZoneLabel$ = that$.next(ITEM_DROPZONE).find(".apex-item-file-dropzone-label");
                    files && files.length > 1 ? fileName = (that$.data("multiple-caption") || "").replace("{count}", files.length) : files[0] && (fileName = files[0].name), fileName ? dropZoneLabel$.text(fileName) : dropZoneLabel$.text(that$.data("caption"))
                });
                ...
                scrollToAnchor(), $(window).on("hashchange", function(e) {
                    e.preventDefault(), scrollToAnchor()
                });
                var hidePagination = function(regionId) {
                    var pagination$ = $("#" + regionId).find(".t-Report-pagination");
                    pagination$.find("td").length > 0 && !(pagination$.find("td.pagination > a").length > 0 || pagination$.find("td.pagination > select").length > 0) && pagination$.addClass("u-hidden")
                };
                if ($(".t-Report--hideNoPagination").each(function() {
                        var regionId = $(this).data("region-id");
                        hidePagination(regionId), $("#" + regionId).on("apexafterrefresh", function() {
                    ...
                    var stickyMobileHeader = function() {
                        var title$ = ut.pageTitle$,
                            stickyMobileTitle = function() {
                                var isSticky = title$.data("apexStickyWidget") ? !0 : !1;
                                ut.util.mq("(max-width: 640px)") ? isSticky || title$.stickyWidget() : isSticky && (title$.stickyWidget("destroy"), setTimeout(ut.resetHeaderOffset, 50))
                            };
                        stickyMobileTitle(), $(window).on("theme42layoutchanged apexwindowresized", stickyMobileTitle)
                    };
                    stickyMobileHeader()
                }
                var navTabs$ = $(".t-NavTabs"),
                    getNavTabsHeight = function(padding) {
                        ...
                        return ut.util.mq("(max-width: 768px)") && (navHeight = navTabs$.outerHeight() || 0), navHeight > 0 && (navHeight += navPadding), navHeight
                    },
        ...
        ut.modalAutoSize = function(elem) {
                ...
                var l$ = apex.util.getTopApex().jQuery,
                    ...
                    dialogReadyClass = function() {
                        $("body").addClass("js-dialogReady"), box$.addClass("js-dialogReady")
                    };
                if (0 !== $(".ui-dialog--stretch").length) return box$.parent().css({
                    height: "95%",
                    width: "95%"
                }), reCenter(), dialogReadyClass(), null;
                ...
                if (isResizeHeightRequired) {
                    ...
                    box$.parent().css("height", newHeight), hasMoved(boxId) || reCenter(), ut.dialogHeight = newHeight, ut.observeModalSize(ut.T_MODAL_BODY, ut.modalAutoSize)
                }
                dialogReadyClass()
            }, ut.observeModalSize = function(elem, callback) {
                var debounceMutationObserver, target = document.querySelector(elem),
                    config = {
                        attributes: !0,
                        childList: !0,
                        characterData: !0,
                    ...
                    observer = new MutationObserver(function(mutations) {
                        clearTimeout(debounceMutationObserver), debounceMutationObserver = setTimeout(act, 50)
                    });
                return observer.observe(target, config), observer
            },
            function() {
                ...
                $.fn.utTabs = function(options) {
                    ...
                    that$.each(function() {
                        ...
                        items$.prepend(ul$), items$.children().filter("div").each(function() {
                            var tab$ = $(this),
                                tabId = tab$.attr("id"),
                ...
                var pushModal, RIGHT_CONTROL_BUTTON = "#t_Button_rightControlButton",
                    A_CONTROLS = "aria-controls",
                    A_EXPANDED = "aria-expanded",
                    A_HIDDEN = "aria-hidden",
                    TREE_NAV_WIDGET_KEY = "nav",
                    RIGHT_WIDGET_KEY = "right",
                    isCollapsedByDefault = $("#t_TreeNav").hasClass("js-defaultCollapsed"),
                    toggleWidgets = {},
                    resetActionsColumn = function() {
                        ut.pageBody$.hasClass("t-PageBody--showLeft") && ut.util.mq("(max-width: 992px)") ? expandWidget(RIGHT_WIDGET_KEY) : ut.util.mq("(max-width: 640px)") && collapseWidget(RIGHT_WIDGET_KEY)
                    },
                    ...
                    buildToggleWidget = function(options) {
                        var widget, checkForElement = options.checkForElement,
                            key = options.key,
                            button$ = $(options.buttonId),
                            expandOriginal = options.onExpand,
                            collapseOriginal = options.onCollapse,
                            element$ = $(checkForElement);
                        return !element$ || element$.length <= 0 ? !1 : (options.controllingElement = button$, button$.attr(A_CONTROLS, element$.attr("id")), options.content = ut.pageBody$, options.contentClassExpanded = "js-" + key + "Expanded", options.contentClassCollapsed = "js-" + key + "Collapsed", options.onExpand = function() {
                            expandOriginal(), button$.addClass("is-active").attr(A_EXPANDED, "true"), pushModal.notify()
                        }, options.onCollapse = function() {
                            collapseOriginal(), button$.removeClass("is-active").attr(A_EXPANDED, "false"), pushModal.notify()
                        }, widget = ToggleCore(options), toggleWidgets[key] = widget, !0)
                    },
                    initialize = function() {
                        if (!(ut.pageBody$.length <= 0 && ut.mainBody$.length <= 0 && ut.header$.length <= 0 && ut.bodyContent$.length <= 0)) {
                            ...
                            $("body").append(pushModal$), $(window).bind("apexwindowresized", function() {
                                for (var key in toggleWidgets) toggleWidgets.hasOwnProperty(key) && toggleWidgets[key].resize();
                                pushModal.notify()
                            }), pushModal = {
                                el$: pushModal$,
                                collapse: function() {},
                            ...
                            var treeShouldBeHidden = function() {
                                ...
                                handleTreeVisibility = function() {
                                    ...
                                    !screenIsTooSmallForTheTree || treeIsHidden() || treeIsHiding ? screenIsTooSmallForTheTree || showTree() : (treeIsHiding = !0, setTimeout(function() {
                                        treeIsHiding = !1, toggleWidgets[TREE_NAV_WIDGET_KEY].isExpanded() || treeNav$.css("visibility", "hidden").attr(A_HIDDEN, "true")
                                    }, 400), $(".t-Body-main").off("click", collapseTree))
                                },
                                hasTree = buildToggleWidget({
                                    ...
                                    onClick: function() {
                                        ut.util.mq("(max-width: 992px)") && RIGHT_WIDGET_KEY in toggleWidgets && toggleWidgets[RIGHT_WIDGET_KEY].isExpanded() && toggleWidgets[RIGHT_WIDGET_KEY].toggle()
                                    },
                                    onExpand: function() {
                                        ut.util.mq("(max-width: 992px)") && collapseWidget(RIGHT_WIDGET_KEY), treeNav$.treeView("expand", treeNav$.treeView("getSelection")), showTree(), ut.delayResize(), treeNav$.trigger("theme42layoutchanged", {
                                            action: "expand"
                                        }), ut.util.mq("(min-width: 640px)") || $(".t-Body-main").on("click", collapseTree)
                                    },
                                    onCollapse: function() {
                                        treeNav$.treeView("collapseAll"), ut.delayResize(), handleTreeVisibility(), treeNav$.trigger("theme42layoutchanged", {
                                            action: "collapse"
                                        }), $(".t-Body-main").off("click", collapseTree)
                                    },
                                    onResize: function() {
                                        var usingTreeNav = pageBody$.hasClass("t-PageBody--leftNav");
                                        usingTreeNav && (ut.util.mq("(max-width: 992px)") ? this.collapse() : this.doesUserPreferExpanded() && this.expand()), handleTreeVisibility(), ut.resetHeaderOffset(), resetActionsColumn()
                                    },
                                    onInitialize: function() {
                                        var preferExpanded = this.doesUserPreferExpanded();
                                        ut.util.mq("(min-width: 992px)") && preferExpanded ? this.expand() : this.forceCollapse()
                                    }
                                });
                            if (hasTree) treeNav$.on("treeviewexpansionstatechange", function(jqueryEvent, treeViewEvent) {
                                treeViewEvent.expanded && toggleWidgets[TREE_NAV_WIDGET_KEY].expand()
                            });
                            else {
                                var headerCore = null,
                                    peekHeaderInit = function() {
                                        if (ut.util.mq("(max-width: 640px)") && null === headerCore) {
                                            var lastScrollTop = 0,
                                                handlerId = null,
                                                recal = function() {
                                            ...
                                            var peekingHeader = function() {
                                                ...
                                                lastScrollTop > scrollTop || 100 > scrollTop ? headerCore.expand() : headerCore.collapse(), lastScrollTop = scrollTop
                                            };
                                            $(window).scroll(function() {
                                                handlerId || (handlerId = apex.util.invokeAfterPaint(peekingHeader))
                                            })
                                        }
                                    };
                                $(window).on("apexwindowresized", peekHeaderInit), peekHeaderInit();
                                var body$ = $("body");
                                body$.hasClass("t-PageBody--noNav") || body$.addClass("t-PageBody--topNav"), $(window).on("apexwindowresized", ut.resetHeaderOffset)
                            }
                            ...
                            buildToggleWidget({
                                ...
                                onClick: function() {
                                    ut.util.mq("(max-width: 992px)") && TREE_NAV_WIDGET_KEY in toggleWidgets && toggleWidgets[TREE_NAV_WIDGET_KEY].isExpanded() && toggleWidgets[TREE_NAV_WIDGET_KEY].toggle()
                                },
                                onExpand: function() {
                                    ut.util.mq("(max-width: 992px)") && pageBody$.hasClass("js-navExpanded") && collapseWidget(TREE_NAV_WIDGET_KEY), actionsContent$.css("visibility", "inherit").attr(A_HIDDEN, "false"), ut.delayResize()
                                },
                                onCollapse: function() {
                                    ut.delayResize(), actionsContent$.attr(A_HIDDEN, "true"), setTimeout(function() {
                                        toggleWidgets[RIGHT_WIDGET_KEY].isExpanded() || actionsContent$.css("visibility", "hidden")
                                    }, 400)
                                },
                                onResize: function() {
                                    this.doesUserPreferExpanded() && !ut.util.mq("(max-width: 640px)") ? this.expand() : this.collapse()
                                },
                                onInitialize: function() {
                                    TREE_NAV_WIDGET_KEY in toggleWidgets && toggleWidgets[TREE_NAV_WIDGET_KEY].isExpanded() && ut.util.mq("(max-width: 992px)") ? this.forceCollapse() : this.doesUserPreferExpanded() ? this.forceExpand() : this.forceCollapse()
                                }
                            }), resetActionsColumn();
                            for (var key in toggleWidgets) toggleWidgets.hasOwnProperty(key) && toggleWidgets[key].initialize()
                        }
                    };
                return {
                ...
                backToTop$.attr("title", apex.lang.getMessage("APEX.UI.BACK_TO_TOP")).click(function() {
                    return $("html,body").animate({
                        scrollTop: 0
                    }, 500), $("a.t-Header-logo-link").focus(), !1
                })
            }, init.dialogAutoHeight = function() {
                var obs, dlg$ = $(".js-dialog-autoheight"),
                    setAutoHeight = function(id) {
                        var elem$ = $(id),
                        ...
                        if (newHeight && 0 !== newHeight) {
                            ...
                            elem$.parent().css("height", newHeight), elem$.css("height", "auto"), hasMoved(id) || elem$.dialog("option", "position", "center"), obs || (obs = ut.observeModalSize(id, setAutoHeight)), elem$.on("dialogresizestart dialogclose", function() {
                                null !== obs && (obs.disconnect(), obs = null)
                            })
                        }
                    };
                ...
                var CL_ACTIVE = "is-active",
                    ...
                    containers$ = $(CL_FLOAT),
                    tags = CL_FLOAT + " .apex-item-text, " + CL_FLOAT + " .apex-item-select, " + CL_FLOAT + " .apex-item-textarea",
                    lov_cal = CL_FLOAT + " .apex-item-popup-lov, " + CL_FLOAT + " .apex-item-datepicker",
                    needLabel = function(item$) {
                        return item$.val() || item$.attr("placeholder") || item$.children("option").first().text()
                    },
                ...
                $(CL_FLOAT + CL_PRE).each(function() {
                    ...
                    var cont$ = $(this);
                    cont$.find("span.apex-item-display-only").length > 0 && cont$.addClass("apex-item-wrapper--display-only")
                }), $(tags).on("focus change", function(e) {
                    var that$ = $(this);
                    that$.val() ? "change" === e.type && shrink(that$) : shrink(that$)
                }).each(function() {
                    var item$ = $(this),
                        label$ = item$.closest(CL_CONTAINER);
                    needLabel(item$) ? label$.addClass(CL_SHOW_LABEL) : label$.removeClass(CL_SHOW_LABEL), item$.is(":disabled") ? label$.addClass(CL_DISABLED) : label$.removeClass(CL_DISABLED), item$.attr("required") && label$.addClass(CL_REQUIRED)
                }).not(lov_cal).on("blur", function() {
                    var item$ = $(this),
                        label$ = item$.closest(CL_CONTAINER).removeClass(CL_ACTIVE);
                    needLabel(item$) ? label$.addClass(CL_SHOW_LABEL) : label$.removeClass(CL_SHOW_LABEL)
                }), $(lov_cal).on("change", function() {
                    var item$ = $(this),
                        label$ = item$.closest(CL_CONTAINER).addClass(CL_SHOW_LABEL);
                    needLabel(item$) || label$.removeClass(CL_SHOW_LABEL + " " + CL_ACTIVE)
                }).each(function() {
                    var that$ = $(this);
                    that$.closest(CL_CONTAINER).hasClass(CL_HTML5_DATE) || shrink(that$)
                })
                ...
                if (!($(".t-BreadcrumbRegion--compactTitle").length > 0 || $(".t-BreadcrumbRegion").length < 0 || pageTitle$.length < 0) && $.trim(pageTitle$.html())) {
                    var expandRecal = ut.resetHeaderOffset,
                        collapseRecal = function() {
                            ut.resetHeaderOffset(), $(".t-Body-info .apex-rds-container").css({
                ...
                var current, maximizeKey = 0,
                    ...
                    hideAllExceptChildren = function(content$) {
                        maximizableRegions$.css("visibility", "hidden"), content$.css("visibility", "visible").find(".js-showMaximizeButton").css("visibility", "visible")
                    },
                    makeCurrent = function(core, content$, top) {
                        ...
                        if (current) {
                            var old = current;
                            current.next = buildCurrent(), current = current.next, current.previous = old
                        } else current = buildCurrent(), ut.pageBody$.addClass("js-regionIsMaximized");
                        apex.theme.defaultStickyTop = top, hideAllExceptChildren(current.content$)
                    };
                maximizableRegions$.length > 0 && applyJqueryUiFocusableFix(), maximizableRegions$.each(function() {
                    var fthOnResize, content$ = $(this),
                        isIRR = content$.hasClass("t-IRR-region"),
                        injectButtonSelector = ".js-maximizeButtonContainer";
                    isIRR && (injectButtonSelector = ".a-IRR-buttons", content$.find(injectButtonSelector).length <= 0 && content$.find(".a-IRR-toolbar").append("<div class='a-IRR-buttons'></div>"));
                    ...
                    var fthOnResizeDebouncer, switchToPrevious = function() {
                        ...
                        forceIRRHeight = function() {
                                ...
                                setTimeout(function() {
                                    ...
                                    if (fthBody$.length > 0) {
                                        ...
                                        var head = safeHeight(content$.find(".t-fht-thead")),
                                            pagWrap = safeHeight(content$.find(".a-IRR-paginationWrap")),
                                            irrToolBar = safeHeight(content$.find(".a-IRR-toolbar")),
                                            controlsContainer = safeHeight(content$.find(".a-IRR-controlsContainer"));
                            ...
                            }, getCollapsible().on("collapsibleexpand", fthOnResize).on("collapsiblecollapse", fthOnResize), $(window).on("apexwindowresized", fthOnResize)
                        },
                        disableForcedIrrHeight = function() {
                            current && isIRR && content$ && (resetIRRHeight(content$.find(".t-fht-tbody")), $(window).off("apexwindowresized", fthOnResize), getCollapsible().off("collapsibleexpand", fthOnResize).off("collapsiblecollapse", fthOnResize))
                        },
                        ...
                        maximizeCore = ToggleCore({
                            ...
                            onExpand: function() {
                                apex.navigation.beginFreezeScroll(), maximizeButton$.attr("title", apex.lang.getMessage("RESTORE")).attr("aria-label", apex.lang.getMessage("RESTORE")).attr("aria-expanded", !0).find(".t-Icon").removeClass("icon-maximize").addClass("icon-restore");
                                var scrollParent$, top = function() {
                                    var height = header$.outerHeight();
                                    return height ? height : 0
                                };
                                isIRR ? (scrollParent$ = content$, forceIRRHeight(), content$.find(".container").first().hide()) : scrollParent$ = content$.find(".t-Region-bodyWrap").first(), content$.find(".js-stickyWidget-toggle").stickyWidget("forceScrollParent", scrollParent$), forceResize(), makeCurrent(maximizeCore, content$, top)
                            },
                            onCollapse: function() {
                                apex.navigation.endFreezeScroll(), maximizeButton$.attr("title", apex.lang.getMessage("MAXIMIZE")).attr("aria-label", apex.lang.getMessage("MAXIMIZE")).attr("aria-expanded", !1).find(".t-Icon").addClass("icon-maximize").removeClass("icon-restore"), disableForcedIrrHeight(), isIRR && content$.find(".container").first().show(), forceResize(), switchToPrevious()
                            }
                        });
                    maximizeCore.initialize()
                }), $(document).on("keydown", function(event) {
                    if (current) {
                        if (event.which === $.ui.keyCode.ESCAPE) return current.core.collapse(), event.preventDefault(), !1;
                        event.which === $.ui.keyCode.TAB && (event.shiftKey && event.target === current.first ? (event.preventDefault(), current.first.focus()) : event.shiftKey || current.last === event.target && (event.preventDefault(), current.last.focus()))
                    }
                })
                ...
                0 !== treeNav$.length && (treeNav$.treeView({
                    ...
                    navigation: !0,
                    autoCollapse: !0
                }), treeNav$.treeView("getSelection").parents().children(".a-TreeView-content").addClass("is-current"), treeNav$.treeView("getSelection").parents(".a-TreeView-node--topLevel").children(".a-TreeView-content, .a-TreeView-row").removeClass("is-current").addClass("is-current--top"), $(".t-TreeNav .a-TreeView-node--topLevel > .a-TreeView-content").each(function() {
                    $(this).find(".fa").length <= 0 && $(this).prepend('<span class="fa fa-file-o"></span>')
                }), ut.renderBadges($(".a-TreeView-label"), "a-TreeView-badge"), treeNav$.on("treeviewexpansionstatechange", function(jqueryEvent, treeViewEvent) {
                ...
                onTheme42Ready: function() {
                    ...
                    }), ut.sticky(".t-Body-contentInner .t-StatusList .t-StatusList-blockHeader"), $(".t-Body-contentInner .t-Report-tableWrap").setTableHeadersAsFixed(), ut.sticky(".js-stickyTableHeader"), rds$.aTabs("option", "showAllScrollOffset", function() {
