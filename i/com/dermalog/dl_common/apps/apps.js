        ...
        if (_SESSION_INFOS.SECONDS_LEFT <= 0) {
            ...
            if (isHandled4OnExpired !== true) {
                ...
                if (lOnExpiredActionFlag === 1) {
                    ...
                    try {
                        dermalog.dl_common.apps.confirm(lMsg, function(pOkPressed) {
...
(function(pApps) {
    ...
    pApps.confirm = function(pMessage, pCallback, pOptions = {}) {
        ...
        setTimeout(function() {
            ...
            setTimeout(function() {
                ...
                dfd.resolve({
                    Result: RETURN_CODES4NAME.SUCCESS,
                    Message: "",
                    JqDialog: jqDialog
                });
            }, 250 + 100);
    ...
    pApps.ApexShowError = function(pErrorData, pOptions, pPreserveMsgCnt = 0) {
        ...
        if (typeof pOptions === "object" && pOptions !== null) {
            ...
            oOptions.callBacks = (typeof pOptions.callBacks === "undefined" ? {
                onClickClose: null
            } : pOptions.callBacks);
        ...
        if (Array.isArray(aErrorItems)) {
            if (aErrorItems.length > 0) {
                ...
                if (nHideAfterSeconds > -1) {
                    nTimer4Items = setTimeout(onTimer4Items, nHideAfterSeconds * 1000);
                }
            }
        }
        if (Array.isArray(aErrorMessages)) {
            if (aErrorMessages.length > 0) {
                ...
                if (oOptions.callBacks) {
                    if (oOptions.callBacks.onClickClose) {
                        ...
                        $("#APEX_ERROR_MESSAGE #t_Alert_Notification .t-Alert-buttons button.t-Button--closeAlert")
                            .on("mouseover", function( /*pEvent*/ ) {
                                isMouseOver = true;
                            })
                            .on("mouseleave", function( /*pEvent*/ ) {
                                isMouseOver = false;
                            })
                            .on("click", (pEvent) => {
                                if (isMouseOver === true) {
                ...
                if (nHideAfterSeconds > -1) {
                    nTimer4Messages = setTimeout(onTimer4Messages, nHideAfterSeconds * 1000);
                }
            }
        }
    };
    ...
    pApps.ApexShowAlert = function(pMsg, pOptions) {
        ...
        if (pApps.closeUnneededAlertDlgs() === 0) {
            pApps.alert(pMsg, lOptions.callback, {
                id: lOptions.id
            });
            /*
            if (lOptions.id) {
        ...
        $.get(pUrl)
            .done(function(pData, pTextStatus, pJqXHR) {
                if (typeof pData === "string" && pData.match(/class="ErrorPageMessage"/)) {
                    dfdResult.reject({
                        Result: RETURN_CODES4NAME.ERROR,
        ...
        oSelf.setup = function(pSrcImg, pJqParent) {
            ...
            if (jqOZI === null) {
                ...
                jqOZI_Img.on("load", function() {
                    ...
                    let oParentDims = {
                        left: oParentOffset.left,
                        top: oParentOffset.top
                    };
        ...
        if (iResult > 0) {
            jqoRoleAlertDlgs.each(function(pIdx4Dlg, pElem4DlgParent) {
                ...
                if (jqoDlg.dialog("isOpen") === true) {
                    ...
                    if (!!sTitle && sTitle.match(/(reload page)/gi) || sMessage.match(/(reload page)/gi) || jqoApexSessAlertDlg.length > 0) {} else {
                        jqoDlg.dialog("close");
                        /*
            ...
            if (iResult > 0) {
                jqoRoleAlertDlgs.each(function(pIdx4Dlg, pElem4DlgParent) {
                    ...
                    if (jqoDlg.dialog("isOpen") === true) {
                        let jqoApexSessAlertDlg = jqoDlgParent.find("#apex_session_alert_dlg");
                        if (jqoApexSessAlertDlg.length > 0) {} else {
                            jqoDlg.dialog("close");
                        }
                    }
                });
     ...
     * @memberOf pApps
     */
    pApps.setLocalizedMessages = function(pMessages, pOptions = {
        serverProcess: "GET_JS_MESSAGES"
    }) {
        return dermalog.dl_common.apps.ApexServerProcess(pOptions.serverProcess, null, {
            dataType: "json",
            success: function(pData, pTextStatus, pjqXHR) {
